...
// حدود المستخدم — القرار قراره لا قرارنا.
// خياران يفهمهما: الوقت (١٠ دقائق حدًّا أدنى، وصفرٌ يعني مفتوحًا) والكلفة
// (دولارٌ حدًّا أدنى، والسقف ما شاء). أما الجولات والاستدعاءات والرموز فليست
// خيارات بل **حرّاس انفلات**، وتتمدد مع مضاعف الكلفة كي لا يوقف حارسٌ
// داخلي مهمةً طويلة رضي صاحبها بثمنها — ولأن بعض المزوّدين لا يبلّغ
// كلفةً أصلًا فيبقى حارس الرموز هو الحارس المالي الفعلي، فيتمدد بالنسبة نفسها.
function userLimits(profile = {}) {
  const cost = Math.max(1, Number(profile.limitCostUsd) || 1);
  const rawMin = Number(profile.limitTimeMin);
  const mins = rawMin === 0 ? 0 : Math.max(10, rawMin || 10);
  return {
    costUsd: cost,
    timeMs: mins === 0 ? Infinity : mins * 60 * 1000,
    rounds: Math.round(DEFAULT_LIMITS.rounds * cost),
    toolCalls: Math.round(DEFAULT_LIMITS.toolCalls * cost),
    tokens: Math.round(DEFAULT_LIMITS.tokens * cost),
  };
}
...
async function runLoop(o) {
  ...
  for (let round = 1; ; round++) {
    ...
    // الصور التي طلبها view_image تُسلَّم هنا.
    // نتيجة الأداة **نصٌّ لا يحمل صورة** في واجهة OpenAI، فتُلحق الصور
    // برسالة مستخدم مستقلة بعد نتائج الجولة — وهذا ما يجعل صورة في مجلدك
    // تُرى كما تُرى الصورة التي ترفقها بيدك.
    if (o.ctx && o.ctx.pendingImages && o.ctx.pendingImages.length) {
      const imgs = o.ctx.pendingImages.splice(0);
      o.messages.push({
        role: 'user',
        content: [
          { type: 'text', text: `الصور المطلوبة: ${imgs.map((i) => i.rel).join('، ')}` },
  ...
  // خاتمة حدود الميزانية.
  // «تجاوز حد الكلفة» وحدها لا تقول شيئًا: كم أُنفق؟ وما الحدّ؟ وماذا أفعل؟
  // — وقد سأل صاحبُ التطبيق نفسه «ماذا تعني». فالرسالة تحمل الرقم والحدّ
  // والطريق: أن يطلب المتابعة، فيبدأ تشغيلٌ جديد بميزانيته.
  const spent = `${budget.cost.toFixed(2)}$`;
  const mins = Math.round((Date.now() - budget.start) / 60000);
  const closers = {
    rounds: `\n\n**توقّفت عند حدّ الجولات** (${limits.rounds} جولة) والمهمة لم تكتمل. قل «أكمل» لأتابع من حيث وقفت.`,
    time: `\n\n**توقّفت عند حدّ الوقت** (${mins} دقيقة) والمهمة لم تكتمل. قل «أكمل» لأتابع — أو ارفع حد الوقت من الإعدادات ⚙️ للمهام الطويلة.`,
    tokens: `\n\n**توقّفت عند حدّ الرموز** والمهمة لم تكتمل. قل «أكمل» لأتابع من حيث وقفت — أو ارفع حد الكلفة من الإعدادات ⚙️ فيتسع معه هذا الحد.`,
    cost: `\n\n**توقّفت عند حدّ الكلفة**: أنفق هذا التشغيل ${spent} وحدُّه ${limits.costUsd}$ للتشغيل الواحد — حارسٌ يمنع طلبًا واحدًا من استنزاف رصيدك.\n\nالمهمة لم تكتمل. قل «أكمل» فأتابع من حيث وقفت بميزانية جديدة، أو ارفع الحد من الإعدادات ⚙️، أو اطلب نموذجًا أرخص للمهام الطويلة.`,
  };
...
module.exports = { runLoop, DEFAULT_LIMITS, userLimits, Cancelled };
