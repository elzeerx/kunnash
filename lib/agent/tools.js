...
const { normalize: normalizeAr } = require('../arabic');
...
const TOOLS = [
  ...
  {
    name: 'search_files',
    description: 'ابحث في محتوى الملفات النصية — المطابقة تتجاهل التشكيل وهمزات الألف، فـ«الاسبوع» تجد «الأسبوع». مثال: {"query":"المورد الرئيسي","glob":"**/*.md"}',
    permission: 'none',
    params: {
      ...
      glob: { type: 'string' },
      regex: { type: 'boolean' },
      exact: { type: 'boolean' },     // مطابقة حرفية بلا تطبيع
    },
    exec({ query, path: startP, glob, regex, exact }, { root }) {
      const { abs: startAbs } = resolveInWorkspace(root, startP || '.', { mustExist: true });
      ...
      // التطبيع مرة واحدة خارج الحلقة — داخلها يعني تطبيع الاستعلام لكل سطر
      const needle = regex || exact ? null : normalizeAr(query);
      if (needle === '') throw new Error('الاستعلام فارغ بعد التطبيع.');
      ...
      for (const f of walkFiles(root, startAbs)) {
        ...
        for (let i = 0; i < lines.length; i++) {
          const hit = matcher ? matcher.test(lines[i])
            : exact ? lines[i].includes(query)
            : normalizeAr(lines[i]).includes(needle);
          if (!hit) continue;
