...
const VIEW_IMG_MAX = 10 * 1024 * 1024;   // بايت — أثقل جزء في الطلب
const VIEW_IMG_PER_RUN = 6;              // صور في التشغيل الواحد
...
const TOOLS = [
  {
    name: 'read_file',
    description: 'اقرأ ملفًا من مساحة العمل — نصًّا أو شيفرة، ويقرأ xlsx وdocx تلقائيًا. مثال: {"path":"تقارير/يوليو.md"}',
    permission: 'none',
    params: {
    ...
    exec({ path: p, offset, limit }, { root }) {
      ...
      // التفريع بالعائلة قبل القراءة.
      // كان الملف يُقرأ utf8 مهما كان: فـxlsx يُخرج عشرين ألف محرف مشوّه
      // (PK♦…) — وهو **أسوأ من خطأ**: يُنفق رموز المستخدم ويُضلّل النموذج
      // بضجيجٍ يظنّه محتوى. والنموذج قد ينادي read_file على أي شيء، فالحارس
      // هنا لا في وصف الأداة.
      const kind = kindOf(abs);
      if (kind === 'sheet') return TOOL_MAP.get('read_excel').exec({ path: p }, { root });
      if (kind === 'doc') return TOOL_MAP.get('read_document').exec({ path: p }, { root });
      if (kind === 'image') return `«${rel}» صورة لا نصّ — استعمل view_image لتراها.`;
      if (kind === 'unsupported' || kind === 'unknown') throw new Error(refusal(abs));
  ...
  {
    // النقل دفعةً واحدة عمدًا: تنظيمُ مجلدٍ مئةُ نقلة، وإذنٌ لكل واحدة
    // مئةُ نقرة تُنهك فيوافق المستخدم بلا نظر — وهذا أسوأ من ألّا نسأل.
    // فالنداء الواحد يحمل الخطة كاملة، ومربع الإذن يعرضها، فيوافق مرة
    // **وقد رآها**. والحذف يبقى على إذنه المنفصل: النقل يُراجَع، والحذف لا.
    name: 'move_files',
    description: 'انقل ملفات أو أعد تسميتها داخل مساحة العمل، دفعةً واحدة. تُنشأ المجلدات الناقصة. مثال: {"moves":[{"from":"تقرير.xlsx","to":"أرشيف/2026/تقرير.xlsx"}]}',
    permission: 'ask',
    params: { moves: { type: 'array', required: true } },
    exec({ moves }, { root }) {
      if (!Array.isArray(moves) || !moves.length) throw new Error('moves مصفوفة فارغة');
      if (moves.length > 200) throw new Error('أكثر من ٢٠٠ نقلة في نداء واحد — قسّمها');

      // نتحقق من الخطة **كلها** قبل تنفيذ أيّ منها: تنفيذٌ نصفه ناجح
      // ونصفه فاشل يترك المجلد في حالٍ لا المستخدم يفهمها ولا نحن.
      const plan = moves.map((m, i) => {
        if (!m || !m.from || !m.to) throw new Error(`النقلة ${i + 1} بلا from أو to`);
        const src = resolveInWorkspace(root, m.from, { mustExist: true, forWrite: true });
        const dst = resolveInWorkspace(root, m.to, { forWrite: true });
        if (fs.existsSync(dst.abs)) throw new Error(`الهدف موجود: ${dst.rel} — لا نطمس ملفًا بالنقل`);
        return { src, dst };
      });
      ...
      for (const { src, dst } of plan) {
        ...
        done.push({ from: src.rel, to: dst.rel });
      }
      return JSON.stringify({ moved: done.length, moves: done });
    },
  },
  {
    name: 'delete_file',
    ...
    exec({ path: p }, { root }) {
      ...
      // «/» لا فاصل النظام: هذه قيمة تُعرض للمستخدم وتُقارن، لا مسار يُفتح
      const movedTo = path.relative(root, target).split(path.sep).join('/');
      return JSON.stringify({ moved_to: movedTo, from: rel });
    },
  },
  {
    name: 'list_files',
  ...
  {
    name: 'view_image',
    // كانت الصور تُرى إن **أرفقتها**، ولا تُرى إن كانت **في مجلدك** — وكُنّاش
    // تطبيقٌ يشتغل على مجلد. فالمرفق يمرّ صورةً في الرسالة، والملف لا سبيل له.
    // هذه الأداة تسوّي الطريقين: تُسجّل الصورة، وتُلحقها الحلقة برسالة مستقلة
    // بعد نتائج الأدوات — لأن نتيجة الأداة نصٌّ لا تحمل صورة في هذه الواجهة.
    description: 'انظر صورة من مساحة العمل (png/jpg/webp/gif). مثال: {"path":"صور/الغلاف.png"}',
    permission: 'none',
    params: { path: { type: 'string', required: true } },
    exec({ path: p }, ctx) {
      const { abs, rel } = resolveInWorkspace(ctx.root, p, { mustExist: true });
      const mime = imageMime(abs);
      if (!mime) throw new Error(`«${rel}» ليست صورة — الصيغ: png وjpg وwebp وgif.`);
      const size = fs.statSync(abs).size;
      if (size > VIEW_IMG_MAX) {
        throw new Error(`«${rel}» أكبر من ١٠ ميجابايت (${Math.round(size / 1048576)}MB) — صغّرها أولًا.`);
      }
      if (!Array.isArray(ctx.pendingImages)) {
        throw new Error('عرض الصور غير متاح في هذا السياق.');
      }
      // سقفٌ للتشغيل الواحد: الصور أثقل الأجزاء، وعشرٌ منها تُتخم الطلب
      if (ctx.pendingImages.length >= VIEW_IMG_PER_RUN) {
        throw new Error(`بلغت حدّ ${VIEW_IMG_PER_RUN} صور في التشغيل الواحد — اكتفِ بما رأيت أو اطلب تشغيلًا جديدًا.`);
      }
      ctx.pendingImages.push({
        rel,
        url: `data:${mime};base64,${fs.readFileSync(abs).toString('base64')}`,
      });
      return `الصورة «${rel}» مرفقةٌ في الرسالة التالية — انظرها ثم أجب.`;
    },
  },
  {
    name: 'read_document',
  ...
  },
  {
    ...
    async exec({ url, max_chars }) {
      // الحارس في safefetch: يفحص العنوان المُترجَم لا الاسم، ويعيد الفحص
      // عند كل إعادة توجيه، ويثبّت ما فُحص في الاتصال نفسه.
      const res = await safeFetch(url);
      const type = String(res.headers['content-type'] || '');
      const text = /html/i.test(type) ? htmlToText(res.body) : res.body;
...
module.exports = { TOOLS, TOOL_MAP, globToRegex, htmlToText };
