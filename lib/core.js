...
const { DEFAULT_CONNECTION, streamChat, testConnection } = require('./connection');
...
function saveConnection(patch) {
  ...
  const { label, baseUrl, model, apiKey } = patch || {};
  ws.saveConnection({ label, baseUrl, model, apiKey });
  return true;
}
function connectionStatus() {
  ...
  return {
    label: c.label,
    model: c.model,
    ready: Boolean(c.apiKey && c.model),
  };
}
...
async function chatSend({ sessionId, text, attachments, retry, requestId }, emit, _onPermission) {
  ...
  try {
    if (!cfg.apiKey) throw new Error('لم يُدخل مفتاح بعد — افتح الإعدادات ⚙️ وأضف مفتاح الاتصال.');
    if (!cfg.model) throw new Error('لم يُحدَّد نموذج بعد — افتح الإعدادات ⚙️ واكتب اسم النموذج.');
...
module.exports = {
  loadConnection, saveConnection, connectionStatus, testConnection,
