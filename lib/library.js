...
function itemPath(root, type, id, d = dirs(root)) {
  if (!ID_RE.test(id)) throw new Error('معرّف غير صالح — استخدم حروفًا لاتينية صغيرة وأرقامًا وشرطات فقط (مثل: monthly-report)');
...
function readItem(root, type, id) {
  for (const d of readDirs(root)) {
    const file = itemPath(root, type, id, d);
...
function saveItem(root, type, id, content) {
  const file = itemPath(root, type, id);
...
module.exports = { listLibrary, readItem, saveItem, deleteItem, templateFor };
