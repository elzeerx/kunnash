...
// ---------- تشفير المفتاح ----------
// المخزَّن: apiKeyEnc (base64 لناتج safeStorage) — أو apiKey نصًا حيث لا تشفير.
function sealKey(conn) {
  if (!cipher || !conn.apiKey) return conn;
  const { apiKey, ...rest } = conn;
  return { ...rest, apiKeyEnc: cipher.encryptString(apiKey).toString('base64') };
}
function unsealKey(conn) {
  if (!conn.apiKeyEnc) return conn;
  const { apiKeyEnc, ...rest } = conn;
  if (!cipher) return rest;   // تشفير بلا مفكّ (لا يقع على ماك) — كأن لا مفتاح
  try {
    return { ...rest, apiKey: cipher.decryptString(Buffer.from(apiKeyEnc, 'base64')) };
  } catch {
    return rest;              // مفتاح من نظام آخر أو تالف — يُعاد إدخاله
  }
}
// مفتاح محفوظ نصًا من قبل توفر التشفير؟ يُشفَّر فور الإقلاع
function migratePlainKey() {
  if (!cipher) return;
  const conn = readConfig().connection || {};
  if (conn.apiKey && !conn.apiKeyEnc) writeConfig({ connection: sealKey(conn) });
}
...
function loadConnection() { return unsealKey(readConfig().connection || {}); }
function saveConnection(patch) {
  const merged = { ...loadConnection() };
  for (const [k, v] of Object.entries(patch || {})) {
    if (v === undefined) continue;          // لم يُرسل الحقل — يبقى كما هو
    if (v === '') delete merged[k];         // مسحه المستخدم — يعود للافتراضي
    else merged[k] = v;
  }
  writeConfig({ connection: sealKey(merged) });
}
